...
find "$dir" -type f -regex '.*\.\(js\|css\)$' -exec sed -i -E -e 's/^\s+//g' \
                                                              -e 's/\s+/ /g' {} \;
# Consecutive line breaks
find "$dir" -type f -regex '.*\.\(js\|css\)$' -exec perl -i -p0e 's,\n+\s*,\n,g' {} \;
...
# DANGER: Collapse whitespace, but not inside <pre> or <textarea>
